module MarketplaceService
  module Inbox
    ...
    module Query
      ...
      def fetch_messages_after(last_viewed_time,conversation_id,person_id)
        # sql = SQLUtils.ar_quote(connection, @fetch_messages_after_sql, last_viewed_time: last_viewed_time,
        #  conversation_id:conversation_id, person_id: person_id)
        messages  = Message.where(conversation:conversation_id).where('created_at > ?', last_viewed_time).all
        MarketplaceService::Conversation::Command.mark_as_read(conversation_id, person_id)
