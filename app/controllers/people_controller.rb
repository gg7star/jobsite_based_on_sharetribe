class PeopleController < Devise::RegistrationsController
  ...
  def create_linkedin_based
    username = UserService::API::Users.username_from_fb_data(
      username: session["devise.linkedin_data"]["username"],
      given_name: session["devise.linkedin_data"]["given_name"],
      family_name: session["devise.linkedin_data"]["family_name"],
      community_id: @current_community.id)
    ...
    person_hash = {
      ...
      :given_name => session["devise.linkedin_data"]["given_name"],
      :family_name => session["devise.linkedin_data"]["family_name"],
      :linkedin_id => session["devise.linkedin_data"]["id"],
      :locale => I18n.locale,
    ...
    # We trust that linkedin has already confirmed these and save the user few clicks
    Email.create!(:address => session["devise.linkedin_data"]["email"], :send_notifications => true, :person => @person, :confirmed_at => Time.now, community_id: @current_community.id)

    @person.set_default_preferences

    @person.store_picture_from_linkedin(session["devise.linkedin_data"]["pictureUrl"])
    ...
    Analytics.record_event(flash, "SignUp", method: :linkedin)
