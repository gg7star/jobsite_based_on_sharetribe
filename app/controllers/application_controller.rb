...
class ApplicationController < ActionController::Base
  ...
  before_filter :check_http_auth,
    ...
    :redirect_account_setup

  before_action :set_last_seen_at, if: proc { person_signed_in? }
  ...
  def fetch_logged_in_user
    if person_signed_in?
      # TODO: Store user type in a cookie, so we don't need that extra query
      if current_person.user_type == Business::PROP_NAME
        @current_user = Business.find(current_person)
      elsif current_person.user_type == Worker::PROP_NAME
        @current_user = Worker.find(current_person)
      elsif current_person.user_type == Buyer::PROP_NAME
        @current_user = Buyer.find(current_person)
      else
  ...
  def ensure_is_worker_or_buyer
    unless @current_user.user_type == Business::PROP_NAME || @current_user.user_type == Worker::PROP_NAME
      redirect_to homepage_with_locale_path, flash: {alert: I18n.t("notifications.only_buyer_worker")}
    end
  end
    ...
    user = Maybe(@current_user).map { |u|
      {
        ...
        online_party_count: @current_user.present? ? MarketplaceService::OnlineStatus.new(@current_user).getStatusList.count : 0,
  ...
  def redirect_account_setup
    return unless @current_user

    # List all paths here when user logged in and account_setup_step isn't 6
    # that should avoid account_setup page
    return if request.params[:controller] == "account_setup" || 
      request.params[:controller] == "account_setup/businesses" || 
      request.params[:controller] == "account_setup/workers" ||
      request.params[:controller] == "account_setup/buyers" ||
      request.params[:controller] == "sessions" && request.params[:action] == "destroy" ||
      request.params[:controller] == "categories" && request.params[:action] == "index" ||
      request.params[:controller] == "community_memberships" && request.params[:action] == "confirmation_pending" ||
      request.params[:controller] == "community_memberships" && request.params[:action] == "pending_consent" ||
      request.params[:controller] == "community_memberships" && request.params[:action] == "give_consent" ||
      request.params[:controller] == "confirmations" ||
      @is_current_community_admin
    # If user hasnt completed the account setup,
    # but has started the account setup stage, he will be redirected to corresponding account setup page
    # if user hasnt started any account setup, he will be redirected to choose which account type he wants to setup
    if @current_user.account_setup_step != Person::ACCOUNT_SETUP_STEPS[:"6"]
      if @current_user.user_type == Business::PROP_NAME
        redirect_to business_account_setup_step1_path
      elsif @current_user.user_type == Worker::PROP_NAME
        redirect_to worker_account_setup_step1_path
      elsif @current_user.user_type == Buyer::PROP_NAME
        redirect_to buyer_account_setup_step1_path
      else
