...
class SessionsController < ApplicationController
  ...
  def linkedin
    ...
    persons = Person
              .includes(:emails, :community_memberships)
              .references(:emails)
              .where(["people.linkedin_id = ? OR emails.address = ?", data.id, data.emailAddress]).uniq

    people_in_this_community = persons.select { |p| p.is_admin? || p.community_memberships.map(&:community_id).include?(@current_community.id) }
    person_by_lk_id = people_in_this_community.find { |p| p.linkedin_id == data.id }
    person_by_email = people_in_this_community.find { |p| p.emails.any? { |e| e.address == data.emailAddress && e.confirmed_at.present? } }
    email_unconfirmed = people_in_this_community.flat_map(&:emails).find { |e| e.address == data.emailAddress && e.confirmed_at.blank? }.present?

    person = person_by_lk_id || person_by_email

    if person
      person.update_linkedin_data(data.id)
      flash[:notice] = t("devise.omniauth_callbacks.success", :kind => "linkedin")
      sign_in_and_redirect person, :event => :authentication
    elsif data.emailAddress.blank?
      flash[:error] = t("layouts.notifications.could_not_get_email_from_linkedin")
      redirect_to sign_up_path and return
    elsif email_unconfirmed
      flash[:error] = t("layouts.notifications.linkedin_email_unconfirmed", email: data.emailAddress)
      redirect_to login_path and return
    else
      linkedin_data = {"email" => data.emailAddress,
                       "given_name" => data.first_name,
                       "family_name" => data.last_name,
                       "username" => data.username,
                       "id"  => data.id,
      ...
      session["devise.linkedin_data"] = linkedin_data
      redirect_to :action => :create_linkedin_based, :controller => :people
    end
  end
