...
class Person < ActiveRecord::Base
  ...
  after_initialize :add_uuid, :set_initial_username, :set_initial_password
  def add_uuid
  ...
  def recieved_offers_count
    return 0 if listings.empty?
    listings.joins(:offers).count
  end

  def recieved_new_offers_count
    return 0 if listings.empty?
    listings.joins(:offers).where(offers: {status: Offer::STATUSES[:pending]}).count
  end

  def latest_recieved_offer
    ordered_listings = listings.joins(:offers).order("offers.created_at desc")
    return 0 if ordered_listings.empty?
  ...
  def store_picture_from_linkedin(url)
    if self.linkedin_id
      self.picture_from_url(url)
    end
  end
  ...
  def update_linkedin_data(linkedin_id)
    self.update_attribute(:linkedin_id, linkedin_id)
    if self.image_file_size.nil?
      self.store_picture_from_linkedin
    end
  end
