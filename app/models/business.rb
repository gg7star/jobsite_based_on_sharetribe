...
class Business < Person
  ...
  has_many :locations, foreign_key: "person_id"
  ...
  attr_accessible(:skills_attributes, :images_attributes, :locations_attributes, :description, :overall_name, :image, :user_type, :account_setup_step)
  ...
  def has_atleast_one_location
    ...
    if account_setup_step >= Person::ACCOUNT_SETUP_STEPS[:"3"]
      if locations.empty?
        errors.add(:locations, "need atleast one locations")
      end
    end
  end
